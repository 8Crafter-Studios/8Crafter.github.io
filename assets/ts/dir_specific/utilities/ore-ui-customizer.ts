...
export namespace OreUICustomizer {
     ...
     * @type {File | undefined}
     */
    export let zipFile: File | undefined = undefined;
    /**
     * @type {(zip.Entry)[] | undefined}
     */
    export let zipFileEntries: zip.Entry[] | undefined = undefined;
    // /**
    //  * @type {zip.BlobWriter}
    //  */
     ...
     * @type {zip.FS}
     */
    export let zipFs: zip.FS | undefined = undefined;
    /**
     * @type {keyof typeof currentPresets}
     */
    export let currentPreset: keyof typeof currentPresets = "none";
    /**
     * @type {File}
     */
    export let currentImportedFile: File | undefined = undefined;

    /**
     * @type {HTMLElement}
     */
    export let currentColorPickerTarget: HTMLElement | undefined = undefined;
    ...
    type CachedLastUsedOreUICustomizerConfigJSON = Omit<
        OreUICustomizerConfig,
        "checkForUpdatesDetails" | "marketplaceDetails" | "metadata" | "oreUICustomizerConfig"
    > & {
        [key in keyof Pick<OreUICustomizerConfig, "oreUICustomizerConfig">]: Omit<
            OreUICustomizerSettings,
            "plugins" | "activePluginsDetails" | "preloadedPlugins"
        >;
    };

    $(function onDocumentLoad() {
        if (localStorage.getItem("ore-ui-customizer-last-config") !== null) {
            let config: CachedLastUsedOreUICustomizerConfigJSON | undefined;
            try {
            ...
            if (config) {
                const settings: CachedLastUsedOreUICustomizerConfigJSON["oreUICustomizerConfig"] = config.oreUICustomizerConfig;
                if (settings) {
                ...
                case "ouicconfig": {
                    try {
                        ...
                        if ("format_version" in config) {
                            ...
                            throw new TypeError(
                                "Invalid config, unsupported config format, missing format_version or oreUICustomizerVersion: " + JSON.stringify(config)
                            );
                        }
                    } catch (e: any) {
                        const popupElement: MessageFormDataElement = document.createElement("mc-message-form-data") as MessageFormDataElement;
        ...
        $(".guiPresetDropdownOption").click(async (event) => {
            ...
            $("#download_in_new_tab_link_open_button").prop("disabled", true);
            $("#download_in_new_tab_link").removeAttr("href");
            currentPreset = selectedInput.value as keyof typeof currentPresets;
        ...
        ($(".spectrum-colorpicker-color-override-option") as JQuery<HTMLInputElement>).each(
            (i, element) =>
        ...
        try {
            ...
            var oreUIPreviewIframeContainer = $("#ore-ui-preview-iframe-container") as JQuery<HTMLDivElement>;
            var oreUIPreviewIframeHeight = oreUIPreviewIframe.outerHeight();
            var oreUIPreviewIframeWidth = oreUIPreviewIframe.outerWidth();

            function resize(event: JQueryEventObject, ui: JQueryUI.ResizableUIParams): void {
                try {
                    var scale, origin;
    ...
    export function detectAndroid(): boolean {
        ...
        if (navigator.platform === "Android") return true;
        if (!navigator.userAgentData) {
            if (navigator.userAgent.includes("Android") && !navigator.userAgent.includes("Windows Phone")) return true;
        }
        return false;
    }

    zip.configure({ useWebWorkers: !detectAndroid() });
    ...
    export function updatePluginsList(): void {
        $("#pluginCount").text(`${encodedImportedPlugins.length} Plugin${encodedImportedPlugins.length !== 1 ? "s" : ""}`);
        $("#plugin-list").children().remove();
        const pluginItemTemplate = $("#plugin-item-template").prop("content");
        for (const plugin of encodedImportedPlugins) {
            ...
            $(pluginItem)
                ...
                .on("click", (): void => {
                    delete importedPlugins[`${plugin.id}-${plugin.uuid}-${plugin.version}`];
                    const encodedPluginIndex: number = encodedImportedPlugins.findIndex((p) => p.uuid === plugin.uuid);
                    encodedPluginIndex !== -1 && encodedImportedPlugins.splice(encodedPluginIndex, 1);
                    updatePluginsList();
                });
     ...
     */
    export async function validateZipFile(): Promise<boolean> {
        /**
         * @type {0 | 1 | 2}
         */
        let failed: 0 | 1 | 2 = 0;
        try {
            ...
            await zipFs.importUint8Array(new Uint8Array(await currentImportedFile!.arrayBuffer()));
        } catch (e) {
            failed = 1;
            console.error(e);
            $("#import_files_error").css("color", "red");
            $("#import_files_error").text(`Invalid zip file. ${e + (e as any)?.stack}`);
            $("#import_files_error").prop("hidden", false);
            return false;
        }
        if (!currentImportedFile) return false;
        try {
            if (!failed && zipFs.entries.findIndex((entry) => entry.getFullname() === "gui" && (entry as zip.ZipDirectoryEntry).directory) === -1) {
                failed = 1;
                if (zipFs.entries.findIndex((entry) => entry.getFullname() === "dist" && (entry as zip.ZipDirectoryEntry).directory) !== -1) {
                    // Repair the zip directory structure.
                    zipFs.move(
                        zipFs.entries.find((entry) => entry.getFullname() === "dist" && (entry as zip.ZipDirectoryEntry).directory)!,
        ...
        $("#download_in_new_tab_link_open_button").prop("disabled", true);
        $("#download_in_new_tab_link").removeAttr("href"); */
                } else if (zipFs.entries.findIndex((entry) => entry.getFullname() === "hbui" && (entry as zip.ZipDirectoryEntry).directory) !== -1) {
                    // Repair the zip directory structure.
                    zipFs.move(
                        zipFs.entries.find((entry) => entry.getFullname() === "hbui" && (entry as zip.ZipDirectoryEntry).directory)!,
        ...
        $("#download_in_new_tab_link").removeAttr("href"); */
            ...
            console.error(e);
            $("#import_files_error").css("color", "red");
            $("#import_files_error").text(`Invalid zip file. Error while parsing directory structure: ${e + (e as any)?.stack}`);
            $("#import_files_error").prop("hidden", false);
        }
        if (failed === 1) {
            $("#apply_mods").prop("disabled", true);
...
*/
    ...
    export function getSettings(hideDependencyWarnings: boolean = false): OreUICustomizerSettings {
        ...
        const resolvedSettings = {
            ...
            colorReplacements: {
                ...
                // "#1e1e1f": "#002c3d",
                // "#8c8d90": "#1fbfff",
                "#a0e081": ($("#colors_customizer_settings_section_green10") as JQuery<HTMLInputElement>).val() ?? "#a0e081",
                "#86d562": ($("#colors_customizer_settings_section_green20") as JQuery<HTMLInputElement>).val() ?? "#86d562",
                "#6cc349": ($("#colors_customizer_settings_section_green30") as JQuery<HTMLInputElement>).val() ?? "#6cc349",
                "#52a535": ($("#colors_customizer_settings_section_green40") as JQuery<HTMLInputElement>).val() ?? "#52a535",
                "#3c8527": ($("#colors_customizer_settings_section_green50") as JQuery<HTMLInputElement>).val() ?? "#3c8527",
                "#2a641c": ($("#colors_customizer_settings_section_green60") as JQuery<HTMLInputElement>).val() ?? "#2a641c",
                "#1d4d13": ($("#colors_customizer_settings_section_green70") as JQuery<HTMLInputElement>).val() ?? "#1d4d13",
                "#153a0e": ($("#colors_customizer_settings_section_green80") as JQuery<HTMLInputElement>).val() ?? "#153a0e",
                "#112f0b": ($("#colors_customizer_settings_section_green90") as JQuery<HTMLInputElement>).val() ?? "#112f0b",
                "#0f2b0a": ($("#colors_customizer_settings_section_green100") as JQuery<HTMLInputElement>).val() ?? "#0f2b0a",
                "#ffffff": ($("#colors_customizer_settings_section_white") as JQuery<HTMLInputElement>).val() ?? "#ffffff",
                "#000000": ($("#colors_customizer_settings_section_black") as JQuery<HTMLInputElement>).val() ?? "#000000",
                "#f4f6f9": ($("#colors_customizer_settings_section_gray10") as JQuery<HTMLInputElement>).val() ?? "#f4f6f9",
                "#e6e8eb": ($("#colors_customizer_settings_section_gray20") as JQuery<HTMLInputElement>).val() ?? "#e6e8eb",
                "#d0d1d4": ($("#colors_customizer_settings_section_gray30") as JQuery<HTMLInputElement>).val() ?? "#d0d1d4",
                "#b1b2b5": ($("#colors_customizer_settings_section_gray40") as JQuery<HTMLInputElement>).val() ?? "#b1b2b5",
                "#8c8d90": ($("#colors_customizer_settings_section_gray50") as JQuery<HTMLInputElement>).val() ?? "#8c8d90",
                "#58585a": ($("#colors_customizer_settings_section_gray60") as JQuery<HTMLInputElement>).val() ?? "#58585a",
                "#48494a": ($("#colors_customizer_settings_section_gray70") as JQuery<HTMLInputElement>).val() ?? "#48494a",
                "#313233": ($("#colors_customizer_settings_section_gray80") as JQuery<HTMLInputElement>).val() ?? "#313233",
                "#242425": ($("#colors_customizer_settings_section_gray90") as JQuery<HTMLInputElement>).val() ?? "#242425",
                "#1e1e1f": ($("#colors_customizer_settings_section_gray100") as JQuery<HTMLInputElement>).val() ?? "#1e1e1f",
                "#ff8080": ($("#colors_customizer_settings_section_red10") as JQuery<HTMLInputElement>).val() ?? "#ff8080",
                "#d93636": ($("#colors_customizer_settings_section_red20") as JQuery<HTMLInputElement>).val() ?? "#d93636",
                "#b31b1b": ($("#colors_customizer_settings_section_red30") as JQuery<HTMLInputElement>).val() ?? "#b31b1b",
                "#d54242": ($("#colors_customizer_settings_section_red40") as JQuery<HTMLInputElement>).val() ?? "#d54242",
                "#ca3636": ($("#colors_customizer_settings_section_red50") as JQuery<HTMLInputElement>).val() ?? "#ca3636",
                "#c02d2d": ($("#colors_customizer_settings_section_red60") as JQuery<HTMLInputElement>).val() ?? "#c02d2d",
                "#b62525": ($("#colors_customizer_settings_section_red70") as JQuery<HTMLInputElement>).val() ?? "#b62525",
                "#ad1d1d": ($("#colors_customizer_settings_section_red80") as JQuery<HTMLInputElement>).val() ?? "#ad1d1d",
                "#a31616": ($("#colors_customizer_settings_section_red90") as JQuery<HTMLInputElement>).val() ?? "#a31616",
                "#990f0f": ($("#colors_customizer_settings_section_red100") as JQuery<HTMLInputElement>).val() ?? "#990f0f",
                "#ffb366": ($("#colors_customizer_settings_section_orange10") as JQuery<HTMLInputElement>).val() ?? "#ffb366",
                "#d3791f": ($("#colors_customizer_settings_section_orange20") as JQuery<HTMLInputElement>).val() ?? "#d3791f",
                "#a65b11": ($("#colors_customizer_settings_section_orange30") as JQuery<HTMLInputElement>).val() ?? "#a65b11",
                "#ffe866": ($("#colors_customizer_settings_section_yellow10") as JQuery<HTMLInputElement>).val() ?? "#ffe866",
                "#e5c317": ($("#colors_customizer_settings_section_yellow20") as JQuery<HTMLInputElement>).val() ?? "#e5c317",
                "#8a7500": ($("#colors_customizer_settings_section_yellow30") as JQuery<HTMLInputElement>).val() ?? "#8a7500",
                "#fff0c5": ($("#colors_customizer_settings_section_gold10") as JQuery<HTMLInputElement>).val() ?? "#fff0c5",
                "#ffd783": ($("#colors_customizer_settings_section_gold20") as JQuery<HTMLInputElement>).val() ?? "#ffd783",
                "#f8af2b": ($("#colors_customizer_settings_section_gold30") as JQuery<HTMLInputElement>).val() ?? "#f8af2b",
                "#ce8706": ($("#colors_customizer_settings_section_gold40") as JQuery<HTMLInputElement>).val() ?? "#ce8706",
                "#ae7100": ($("#colors_customizer_settings_section_gold50") as JQuery<HTMLInputElement>).val() ?? "#ae7100",
                "#8cb3ff": ($("#colors_customizer_settings_section_blue10") as JQuery<HTMLInputElement>).val() ?? "#8cb3ff",
                "#2e6be5": ($("#colors_customizer_settings_section_blue20") as JQuery<HTMLInputElement>).val() ?? "#2e6be5",
                "#1452cc": ($("#colors_customizer_settings_section_blue30") as JQuery<HTMLInputElement>).val() ?? "#1452cc",
                "rgba(0, 0, 0, 0.1)": ($("#colors_customizer_settings_section_blackOpacity10") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.1)",
                "rgba(0, 0, 0, 0.2)": ($("#colors_customizer_settings_section_blackOpacity20") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.2)",
                "rgba(0, 0, 0, 0.25)": ($("#colors_customizer_settings_section_blackOpacity25") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.25)",
                "rgba(0, 0, 0, 0.3)": ($("#colors_customizer_settings_section_blackOpacity30") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.3)",
                "rgba(0, 0, 0, 0.4)": ($("#colors_customizer_settings_section_blackOpacity40") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.4)",
                "rgba(0, 0, 0, 0.5)": ($("#colors_customizer_settings_section_blackOpacity50") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.5)",
                "rgba(0, 0, 0, 0.6)": ($("#colors_customizer_settings_section_blackOpacity60") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.6)",
                "rgba(0, 0, 0, 0.7)": ($("#colors_customizer_settings_section_blackOpacity70") as JQuery<HTMLInputElement>).val() ?? "rgba(0, 0, 0, 0.7)",
                ...
                "#F877DC": ($("#colors_customizer_settings_section_pink40") as JQuery<HTMLInputElement>).val() ?? "#F877DC",
                "#643ACB": ($("#colors_customizer_settings_section_purple40") as JQuery<HTMLInputElement>).val() ?? "#643ACB",
                "#AC90F3": ($("#colors_customizer_settings_section_deepBlue10") as JQuery<HTMLInputElement>).val() ?? "#AC90F3",
                "#9471E0": ($("#colors_customizer_settings_section_deepBlue20") as JQuery<HTMLInputElement>).val() ?? "#9471E0",
                "#8557F8": ($("#colors_customizer_settings_section_deepBlue40") as JQuery<HTMLInputElement>).val() ?? "#8557F8",
                "#7345E5": ($("#colors_customizer_settings_section_deepBlue50") as JQuery<HTMLInputElement>).val() ?? "#7345E5",
                "#5D2CC6": ($("#colors_customizer_settings_section_deepBlue60") as JQuery<HTMLInputElement>).val() ?? "#5D2CC6",
                "#4A1CAC": ($("#colors_customizer_settings_section_deepBlue70") as JQuery<HTMLInputElement>).val() ?? "#4A1CAC",
                "#050029": ($("#colors_customizer_settings_section_deepBlue100") as JQuery<HTMLInputElement>).val() ?? "#050029",
                "rgba(5, 0, 41, 0.5)": ($("#colors_customizer_settings_section_deepBlueOpacity50") as JQuery<HTMLInputElement>).val() ?? "rgba(5, 0, 41, 0.5)",
            },
        };
        encodedImportedPlugins?.forEach((plugin) => {
    ...
    export function setSettings(settings: {
        ...
        // "#1e1e1f": "#002c3d",
        // "#8c8d90": "#1fbfff",
        ($("#colors_customizer_settings_section_green10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#a0e081"] ?? "#a0e081");
        ($("#colors_customizer_settings_section_green20") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#86d562"] ?? "#86d562");
        ($("#colors_customizer_settings_section_green30") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#6cc349"] ?? "#6cc349");
        ($("#colors_customizer_settings_section_green40") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#52a535"] ?? "#52a535");
        ($("#colors_customizer_settings_section_green50") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#3c8527"] ?? "#3c8527");
        ($("#colors_customizer_settings_section_green60") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#2a641c"] ?? "#2a641c");
        ($("#colors_customizer_settings_section_green70") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#1d4d13"] ?? "#1d4d13");
        ($("#colors_customizer_settings_section_green80") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#153a0e"] ?? "#153a0e");
        ($("#colors_customizer_settings_section_green90") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#112f0b"] ?? "#112f0b");
        ($("#colors_customizer_settings_section_green100") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#0f2b0a"] ?? "#0f2b0a");
        ($("#colors_customizer_settings_section_white") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ffffff"] ?? "#ffffff");
        ($("#colors_customizer_settings_section_black") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#000000"] ?? "#000000");
        ($("#colors_customizer_settings_section_gray10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#f4f6f9"] ?? "#f4f6f9");
        ($("#colors_customizer_settings_section_gray20") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#e6e8eb"] ?? "#e6e8eb");
        ($("#colors_customizer_settings_section_gray30") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#d0d1d4"] ?? "#d0d1d4");
        ($("#colors_customizer_settings_section_gray40") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#b1b2b5"] ?? "#b1b2b5");
        ($("#colors_customizer_settings_section_gray50") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#8c8d90"] ?? "#8c8d90");
        ($("#colors_customizer_settings_section_gray60") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#58585a"] ?? "#58585a");
        ($("#colors_customizer_settings_section_gray70") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#48494a"] ?? "#48494a");
        ($("#colors_customizer_settings_section_gray80") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#313233"] ?? "#313233");
        ($("#colors_customizer_settings_section_gray90") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#242425"] ?? "#242425");
        ($("#colors_customizer_settings_section_gray100") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#1e1e1f"] ?? "#1e1e1f");
        ($("#colors_customizer_settings_section_red10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ff8080"] ?? "#ff8080");
        ($("#colors_customizer_settings_section_red20") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#d93636"] ?? "#d93636");
        ($("#colors_customizer_settings_section_red30") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#b31b1b"] ?? "#b31b1b");
        ($("#colors_customizer_settings_section_red40") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#d54242"] ?? "#d54242");
        ($("#colors_customizer_settings_section_red50") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ca3636"] ?? "#ca3636");
        ($("#colors_customizer_settings_section_red60") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#c02d2d"] ?? "#c02d2d");
        ($("#colors_customizer_settings_section_red70") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#b62525"] ?? "#b62525");
        ($("#colors_customizer_settings_section_red80") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ad1d1d"] ?? "#ad1d1d");
        ($("#colors_customizer_settings_section_red90") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#a31616"] ?? "#a31616");
        ($("#colors_customizer_settings_section_red100") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#990f0f"] ?? "#990f0f");
        ($("#colors_customizer_settings_section_orange10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ffb366"] ?? "#ffb366");
        ($("#colors_customizer_settings_section_orange20") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#d3791f"] ?? "#d3791f");
        ($("#colors_customizer_settings_section_orange30") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#a65b11"] ?? "#a65b11");
        ($("#colors_customizer_settings_section_yellow10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ffe866"] ?? "#ffe866");
        ($("#colors_customizer_settings_section_yellow20") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#e5c317"] ?? "#e5c317");
        ($("#colors_customizer_settings_section_yellow30") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#8a7500"] ?? "#8a7500");
        ($("#colors_customizer_settings_section_gold10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#fff0c5"] ?? "#fff0c5");
        ($("#colors_customizer_settings_section_gold20") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ffd783"] ?? "#ffd783");
        ($("#colors_customizer_settings_section_gold30") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#f8af2b"] ?? "#f8af2b");
        ($("#colors_customizer_settings_section_gold40") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ce8706"] ?? "#ce8706");
        ($("#colors_customizer_settings_section_gold50") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#ae7100"] ?? "#ae7100");
        ($("#colors_customizer_settings_section_blue10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#8cb3ff"] ?? "#8cb3ff");
        ...
        ($("#colors_customizer_settings_section_pink30") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#E833C2"] ?? "#E833C2");
        ($("#colors_customizer_settings_section_pink40") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#F877DC"] ?? "#F877DC");
        ($("#colors_customizer_settings_section_purple40") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#643ACB"] ?? "#643ACB");
        ($("#colors_customizer_settings_section_deepBlue10") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#AC90F3"] ?? "#AC90F3");
        ($("#colors_customizer_settings_section_deepBlue20") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#9471E0"] ?? "#9471E0");
        ($("#colors_customizer_settings_section_deepBlue40") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#8557F8"] ?? "#8557F8");
        ($("#colors_customizer_settings_section_deepBlue50") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#7345E5"] ?? "#7345E5");
        ($("#colors_customizer_settings_section_deepBlue60") as JQuery<HTMLInputElement>).val(settings.colorReplacements?.["#5D2CC6"] ?? "#5D2CC6");
     ...
     */
    export function applyColorFilterToColorOverride(
        target: HTMLElement,
        filterOptions: {
            hueShift?: number;
    ...
    export function stringToRegexString(str: string): string {
        return str.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    }

    export function makeRegexRobustToWhitespace(regexString: string): string {
        return regexString.replace(/\s+/g, "\\s*");
    }

    export async function refreshOreUIPreview(menuHTMLFileName = "index_playscreen_worldstab.html") {
        const newData = {
        ...
        for (const k in newData) {
            ...
            const key: keyof typeof newData = k as keyof typeof newData;
        ...
        for (const k in newData) {
            ...
            for (const [color, replacementColor] of Object.entries(settings.colorReplacements)) {
                ...
                newData[key] = newData[key].replaceAll(color, replacementColor as string);
            }
        }
        ...
        const iframe: HTMLIFrameElement = $("#ore-ui-preview-iframe").get(0) as HTMLIFrameElement;
    ...
    export async function applyMods(): Promise<boolean> {
        $("#apply_mods").prop("disabled", true);
        ...
        for (const plugin of encodedImportedPlugins) {
            if (importedPlugins[`${plugin.id}-${plugin.uuid}-${plugin.version}`]) continue;
            importPluginFromDataURI(
                plugin.dataURI,
                {
                    oreUICustomizerEnvGlobalVariableName,
        ...
        $("#current_customizer_status").text("Applying mods (Running plugin global_before actions)...");
        ...
        {
            const entryCount: number = (zipFs.entries as (zip.ZipFileEntry<any, any> | zip.ZipDirectoryEntry)[]).filter(
                (v: zip.ZipFileEntry<any, any> | zip.ZipDirectoryEntry): boolean => v.directory === void false
            ).length;
            let entryIndex = -1;
            ...
            const statusUpdateIntervalId = setInterval((): void => {
                if (lastStatus === lastAppliedStatus) return;
            ...
            try {
                for (const entry of zipFs.entries as (zip.ZipFileEntry<any, any> | zip.ZipDirectoryEntry)[]) {
                    ...
                    if (!filePath) {
                        ...
                        lastStatus = `Applying mods (Modifying files (${entryIndex}/${entryCount}): ${entry.name})...`;
                        ...
                        if (entry.directory !== void false) {
                            ...
                            for (const plugin of plugins) {
                                if (
                                    plugin.namespace !== "built-in" ||
                                    (settings.enabledBuiltInPlugins[plugin.id as keyof typeof settings.enabledBuiltInPlugins] ?? true)
                                ) {
                                    for (const action of plugin.actions) {
                            ...
                            if (failedReplaces.length > 0) allFailedReplaces[filePath] = failedReplaces;
                            if (origData && distData && !arrayBuffersAreEqual(await origData.arrayBuffer(), await distData.arrayBuffer())) {
        ...
        try {
            ...
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/assets/toggle_off.png", await fetch("/assets/oreui/assets/toggle_off.png").then((r) => r.blob()));
            console.log("Added gui/dist/hbui/assets/toggle_off.png");
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/assets/toggle_on_hover.png", await fetch("/assets/oreui/assets/toggle_on_hover.png").then((r) => r.blob()));
            console.log("Added gui/dist/hbui/assets/toggle_on_hover.png");
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/assets/toggle_on.png", await fetch("/assets/oreui/assets/toggle_on.png").then((r) => r.blob()));
            console.log("Added gui/dist/hbui/assets/toggle_on.png");
            addedCount++;
            // Radio
            zipFs.addBlob("gui/dist/hbui/assets/radio_off_hover.png", await fetch("/assets/oreui/assets/radio_off_hover.png").then((r) => r.blob()));
            console.log("Added gui/dist/hbui/assets/radio_off_hover.png");
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/assets/radio_off.png", await fetch("/assets/oreui/assets/radio_off.png").then((r) => r.blob()));
            console.log("Added gui/dist/hbui/assets/radio_off.png");
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/assets/radio_on_hover.png", await fetch("/assets/oreui/assets/radio_on_hover.png").then((r) => r.blob()));
            console.log("Added gui/dist/hbui/assets/radio_on_hover.png");
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/assets/radio_on.png", await fetch("/assets/oreui/assets/radio_on.png").then((r) => r.blob()));
            ...
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/assets/edit_box_indent.png", await fetch("/assets/oreui/assets/edit_box_indent.png").then((r) => r.blob()));
                ...
                `const oreUICustomizerConfig = ${JSON.stringify(
                    {
                        ...
                        activePluginsDetails: [
                            ...(settings.plugins?.map((plugin: EncodedPluginData): NonNullable<OreUICustomizerSettings["activePluginsDetails"]>[number] => ({
                            ...
                            ...(settings.preloadedPlugins?.map((plugin: Plugin): NonNullable<OreUICustomizerSettings["activePluginsDetails"]>[number] => ({
                                format_version: plugin.format_version,
                                id: plugin.id,
                                name: plugin.name,
                                namespace: plugin.namespace,
...
const oreUICustomizerVersion = ${JSON.stringify(format_version)};`
        ...
        try {
            ...
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/JSONB.js.map", await fetch("/assets/oreui/JSONB.js.map").then((r) => r.blob()));
            ...
            addedCount++;
            zipFs.addBlob("gui/dist/hbui/@ore-ui-types/enums.map", await fetch("/assets/oreui/@ore-ui-types/enums.map").then((r) => r.blob()));
        ...
        $("#current_customizer_status").text("Applying mods (Running plugin global actions)...");
        for (const plugin of plugins) {
            if (plugin.namespace !== "built-in" || (settings.enabledBuiltInPlugins[plugin.id as keyof typeof settings.enabledBuiltInPlugins] ?? true)) {
                for (const action of plugin.actions) {
...
<pre>${Object.entries(allFailedReplaces)
    ...
    export async function downloadInNewTab(): Promise<void> {
        if (zipFs === undefined) {
        ...
        try {
            // Disable web workers for improved compatibility, when the user is having issues.
            zip.configure({ useWebWorkers: false });
            const blob = await zipFs.exportBlob({
                async onprogress(progress: number, total: number): Promise<void> {
                    lastStatus = `Compressing modified zip file (${progress}/${total}) (${((progress / total) * 100).toFixed(1)}%)...`;
                },
            });
            clearInterval(statusUpdateIntervalId);
            ...
            $("#import_files_error").css("color", "red");
            $("#import_files_error").text("");
            const newWarning = `<b>An error occurred while generating the download link for the modified zip file:</b>
<pre>${e}</pre>`;
            $("#import_files_error").append(newWarning);
            $("#import_files_error").prop("hidden", false);
            $("#customizer_loading_bar").hide();
    ...
    export async function download(): Promise<void> {
        ...
        try {
            ...
            $("#import_files_error").css("color", "red");
            $("#import_files_error").text("");
            const newWarning = `<b>An error occurred while downloading the modified zip file:</b>
<pre>${e}</pre>`;
            $("#import_files_error").append(newWarning);
            $("#import_files_error").prop("hidden", false);
            $("#customizer_loading_bar").hide();
    ...
    export async function exportAsRP(): Promise<void> {
        if (zipFs === undefined) {
            throw new Error("zipFs is undefined");
        }
                    ...
                    JSON.stringify(
                        {
                            ...
                            header: {
                                ...
                                name: `8Crafter's Ore UI Customizer v${format_version} - ${Date.now()}`,
                                uuid: randomUUID(),
                                version: [
                            ...
                            metadata: {
                                ...
                                generated_with: {
                                    "8CraftersOreUICustomizer": [format_version],
                                },
                            },
                        },
                        null,
        ...
        const iconFileEntry =
            zipFs.find("pack_icon.png") ? null : (
                zipFs.addBlob("pack_icon.png", await fetch("/assets/oreui/assets/ore_ui_customizer_icon.png").then((r) => r.blob()))
            );
...
<pre>${e}</pre>`;
        ...
        if (manifestFileEntry) zipFs.remove(manifestFileEntry);
        if (iconFileEntry) zipFs.remove(iconFileEntry);
    }
}
