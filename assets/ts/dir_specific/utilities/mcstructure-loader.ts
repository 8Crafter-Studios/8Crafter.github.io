 ...
 */
const importedFiles: { [fileIndex: number]: File } = {};

/**
 * @type {{[fileIndex: number]: boolean}}
 */
const importedFilesWithValidNames: { [fileIndex: number]: boolean } = {};
let fileIndex = 0;
 ...
 */
function importFiles(files: File[] | FileList) {
    $("#blankslate").remove();
    ...
    for (let i = 0; i < files.length; i++) {
        if (files[i]?.name.endsWith(".mcstructure")) {
            ...
            importedFiles[fileIndex] = file;
            importedFilesWithValidNames[fileIndex] = true;
            const thisIndex = fileIndex;
            fileIndex++;

            $(fileItem)
                .find("[data-temp='filename']")
                .text(`${file.name} (${formatFileSizeMetric(file.size)})`);
            $(fileItem)
                .find("[data-value='structurename']")
                .val("andexsi:" + file.name.split(".").slice(0, -1).join("."));
...
function checkIfStructureNamesAreValid() {
    for (let i in importedFiles as any[]) {
...
class Zip {
    ...
    constructor(name: string) {
        this.name = name;
        this.zip = new Array() as any;
        this.file = new Array();
    }

    dec2bin = (dec: number, size: number) => dec.toString(2).padStart(size, "0");
    str2dec = (str: string | undefined) => Array.from(new TextEncoder().encode(str));
    str2hex = (str: string | undefined) => [...new TextEncoder().encode(str)].map((x) => x.toString(16).padStart(2, "0"));
    hex2buf = (hex: string) => new Uint8Array(hex.split(" ").map((x: string) => parseInt(x, 16)));
    bin2hex = (bin: string) => parseInt(bin.slice(8), 2).toString(16).padStart(2, "0") + " " + parseInt(bin.slice(0, 8), 2).toString(16).padStart(2, "0");

    reverse = (hex: string | any[]) => {
    ...
    fetch2zip(filesArray: string[], folder = "") {
                ...
                fetch(fileUrl)
                    ...
                    .then((blob) => {
                        new Response(blob).arrayBuffer().then((buffer) => {
                            ...
                            let uint = [...new Uint8Array(buffer)] as number[] & {
                                name: string;
                                modTime: Date | string | null;
                                fileUrl: string;
                            };
    ...
    str2zip(name: string, str: string, folder = "") {
        let uint = [...new Uint8Array(this.str2dec(str))] as number[] & {
            name: string;
            modTime: Date;
            fileUrl: string;
        };
    ...
    async files2zip(files: string | any[], folder = "") {
        for (let i = 0; i < files.length; i++) {
            await files[i].arrayBuffer().then((data: any) => {
                let uint = [...new Uint8Array(data)] as number[] & {
                    name: string;
                    modTime: Date;
                    fileUrl: string;
                };
    ...
    async fileObjects2zip(files: string | any[], folder = "") {
        for (let i = 0; i < files.length; i++) {
    ...
    makeZip() {
        ...
        for (const name in zip) {
            ...
            let size = this.reverse(parseInt(zip[name]?.length.toString()!).toString(16).padStart(8, "0"));
 ...
 * @returns {File}
 */
function createTextFile(text: string, name: string, type: string): File {
    return new File([new Blob([text], { type: type })], name);
}

declare function download(file: File, name: string): void;
globalThis.download = function download(file: File, name: string): void {
...
async function generateStructureImporterPack() {
    ...
    (Object.entries(importedFiles) as any as [number, File][]).forEach(([i, f]) => {
        ...
        JSON.stringify(
            {
                ...
                modules: [
                    {
                        description: `Structure list: ${JSON.stringify(
                            Object.values(importedFiles).map((f) =>
                                f.name.includes("/") ? f.name.slice(0, -12).replace("/", ":") : "mystructure:" + f.name.slice(0, -12)
                            )
                        )}; Created at: ${Date.now()}; Generated using version 1.1.0; This pack was generated at at §bhttps://www.8crafter.com/utilities/mcstructure-loader`,
                        type: "data",
                        uuid: randomUUID(),
