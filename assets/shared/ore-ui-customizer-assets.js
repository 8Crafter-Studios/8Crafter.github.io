 ...
 */
export function getExtractedSymbolNames(fileContents, fileName) {
    /**
    ...
    const showWarnings = /(?:index|gameplay|editor(?:-menu|-project)?)-[0-9a-f]{5,20}\.js$/.test(fileName);
    let failures = [];
    extractedSymbolNames.facetHolder =
        fileContents.match(/inverse:\(0,([a-zA-Z0-9_$])\.useFacetMap\)\(\(\(?([a-zA-Z0-9_$])\)?=>"POP"===(?:[a-zA-Z0-9_$])\),\[\],\[([a-zA-Z0-9_$])\]\)\}\)\)\)/)?.[1] ?? (failures.push("facetHolder"), extractedSymbolNames.facetHolder);
    extractedSymbolNames.contextHolder =
        fileContents.match(/(?:[a-zA-Z0-9_$]{2})=\(\{route:(?:[a-zA-Z0-9_$]),children:(?:[a-zA-Z0-9_$])\}\)=>\{const (?:[a-zA-Z0-9_$])=\(0,([a-zA-Z0-9_$]).useContext\)/)?.[1] ?? (failures.push("contextHolder"), extractedSymbolNames.contextHolder);
    ...
    extractedSymbolNames.headerFunciton =
        fileContents.match(new RegExp(`${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),null,([a-zA-Z0-9_$]{1})\\("\\.microsoftSignInButtonTitle"\\)\\),`))?.[1] ?? (failures.push("headerFunction"), extractedSymbolNames.headerFunciton);
    extractedSymbolNames.headerSpacingFunction =
        fileContents.match(new RegExp(`${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{size:1\\}\\),`))?.[1] ?? (failures.push("headerSpacingFunction"), extractedSymbolNames.headerSpacingFunction);
    ...
    extractedSymbolNames.jsText =
        fileContents.match(new RegExp(`${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{type:"body",variant:"dimmer"\\}`))?.[1] ?? (failures.push("jsText"), extractedSymbolNames.jsText);
    extractedSymbolNames.navbarButtonFunction =
        fileContents.match(new RegExp(`return ${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{className:"`))?.[1] ?? (failures.push("navbarButtonFunction"), extractedSymbolNames.navbarButtonFunction);
    extractedSymbolNames.navbarButtonImageFunction =
        fileContents.match(new RegExp(`${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{className:"[a-zA-Z0-9_$]+",src:[a-zA-Z0-9_$]{2},imageRendering:"pixelated"`))?.[1] ?? (failures.push("navbarButtonImageFunction"), extractedSymbolNames.navbarButtonImageFunction);
    ...
    const replacerRegexes = {
        ...
        hardcoreModeToggleAlwaysClickable: {
            ...
            0: [
                 ...
                 * - 1.21.90.20 preview (index-fe5c0.js)
                 */
                new RegExp(`function ([a-zA-Z0-9_$]{2,3})\\(\\s*?\\{\\s*?generalData\\s*?:\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?,\\s*?isLockedTemplate\\s*?:\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\}\\s*?\\)\\s*?\\{\\s*?const\\s*?\\{\\s*?t\\s*?:\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\}\\s*?=\\s*?([a-zA-Z0-9_$]{2})\\("CreateNewWorld\\.general"\\)\\s*?,\\s*?([a-zA-Z0-9_$]{1})\\s*?=\\s*?([a-zA-Z0-9_$]{2})\\(\\)\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?=\\s*?\\(\\s*?0\\s*?,\\s*?${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\s*?\\.\\s*?useContext\\s*?\\)\\s*?\\(\\s*?([a-zA-Z0-9_$]{2})\\s*?\\)\\s*?===\\s*?([a-zA-Z0-9_$]{2})\\.CREATE\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})=\\s*?\\(\\s*?0\\s*?,\\s*?${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useSharedFacet\\s*?\\)\\s*?\\(\\s*?([a-zA-Z0-9_$]{2})\\s*?\\)\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?=\\s*?\\(\\s*?0\\s*?,\\s*?${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetMap\\s*?\\)\\s*?\\(\\s*?\\(\\s*?\\(\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\)\\s*?=>\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\|\\|\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\|\\|\\s*?!(?:[a-zA-Z0-9_$]{1})\\s*?\\|\\|\\s*?(?:[a-zA-Z0-9_$]{1})\\.gameMode\\s*?!==\\s*?([a-zA-Z0-9_$]{2})\\.SURVIVAL\\s*?&&\\s*?(?:[a-zA-Z0-9_$]{1})\\.gameMode\\s*?!==\\s*?([a-zA-Z0-9_$]{2})\\.ADVENTURE\\s*?\\)\\s*?,\\s*?\\[\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\]\\s*?,\\s*?\\[\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\]\\s*?\\)\\s*?;\\s*?return \\s*?${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(\\s*?([a-zA-Z0-9_$]{2})\\s*?,\\s*?\\{\\s*?title\\s*?:\\s*?([a-zA-Z0-9_$]{1})\\("\\.hardcoreModeTitle"\\)\\s*?,\\s*?soundEffectPressed\\s*?:\\s*?"ui\\.hardcore_toggle_press"\\s*?,\\s*?disabled\\s*?:\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?,\\s*?description\\s*?:\\s*?([a-zA-Z0-9_$]{1})\\("\\.hardcoreModeDescription"\\)\\s*?,\\s*?value\\s*?:\\s*?\\(\\s*?0\\s*?,\\s*?${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetMap\\s*?\\)\\s*?\\(\\s*?\\(\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?=>\\s*?(?:[a-zA-Z0-9_$]{1})\\.isHardcore\\s*?\\)\\s*?,\\s*?\\[\\s*?\\]\\s*?,\\s*?\\[\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\]\\s*?\\)\\s*?,\\s*?onChange\\s*?:\\s*?\\(\\s*?0\\s*?,\\s*?${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetCallback\\s*?\\)\\s*?\\(\\s*?\\(\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?=>\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?=>\\s*?\\{\\s*?(?:[a-zA-Z0-9_$]{1})\\.isHardcore\\s*?=\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?,\\s*?(?:[a-zA-Z0-9_$]{1})\\(\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\?\\s*?"ui\\.hardcore_enable"\\s*?:\\s*?"ui\\.hardcore_disable"\\s*?\\)\\s*?\\}\\s*?\\)\\s*?,\\s*?\\[\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\]\\s*?,\\s*?\\[\\s*?(?:[a-zA-Z0-9_$]{1})\\s*?\\]\\s*?\\)\\s*?,(?:\\s*?gamepad\\s*?:\\s*?\\{\\s*?index\\s*?:\\s*?4\\s*?\\}\\s*?,)?\\s*?imgSrc\\s*?:\\s*?([a-zA-Z0-9_$]{2,3})\\s*?,\\s*?"data-testid"\\s*?:\\s*?"hardcore-mode-toggle"\\s*?\\}\\s*?\\)\\s*?\\}`, "g"),
            ],
        },
        /**
        ...
        allowForChangingFlatWorldPreset: {
            ...
            0: [
                 ...
                 * - \> 1.21.90.21 preview (index-aaad2.js)
                 */
                new RegExp(`${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.Mount,\\{when:(?:[a-zA-Z0-9_$]{1})\\},${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2,3}),\\{value:\\(0,${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetMap\\)\\(\\(e=>e\\.useFlatWorld\\),\\[\\],\\[([a-zA-Z0-9_$]{1})\\]\\),preset:\\(0,${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetMap\\)\\(\\(e=>e\\.flatWorldPreset\\),\\[\\],\\[(?:[a-zA-Z0-9_$]{1})\\]\\),onValueChanged:\\(0,${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetCallback\\)\\(\\(e=>t=>\\{e\\.useFlatWorld=t,t&&e\\.flatWorldPreset\\?([a-zA-Z0-9_$]{1})\\(([a-zA-Z0-9_$]{2,3})\\[e\\.flatWorldPreset\\]\\):(?:[a-zA-Z0-9_$]{1})\\(""\\)\\}\\),\\[(?:[a-zA-Z0-9_$]{1})\\],\\[(?:[a-zA-Z0-9_$]{1})\\]\\),onPresetChanged:\\(0,${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetCallback\\)\\(\\(e=>t=>\\{e\\.flatWorldPreset=t,e\\.useFlatWorld\\?(?:[a-zA-Z0-9_$]{1})\\((?:[a-zA-Z0-9_$]{2,3})\\[t\\]\\):(?:[a-zA-Z0-9_$]{1})\\(""\\)\\}\\),\\[(?:[a-zA-Z0-9_$]{1})\\],\\[(?:[a-zA-Z0-9_$]{1})\\]\\),disabled:(?:[a-zA-Z0-9_$]{1}),hideAccordion:\\(0,${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.useFacetMap\\)\\(\\(e=>null==e\\.flatWorldPreset\\),\\[\\],\\[(?:[a-zA-Z0-9_$]{1})\\]\\),achievementsDisabledMessages:([a-zA-Z0-9_$]{1})\\}\\)\\)\\)`),
            ],
            /**
            ...
            1: [
                 ...
                 */
                {
                    regex: new RegExp(`return ${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.Fragment,null,${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.Mount,\\{when:(?:[a-zA-Z0-9_$]{1})\\},${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{onChange:([a-zA-Z0-9_$]{1}),value:([a-zA-Z0-9_$]{1}),title:([a-zA-Z0-9_$]{1})\\("\\.useFlatWorldTitle"\\),description:(?:[a-zA-Z0-9_$]{1})\\("\\.useFlatWorldDescription"\\),disabled:([a-zA-Z0-9_$]{1}),offNarrationText:([a-zA-Z0-9_$]{1}),onNarrationText:([a-zA-Z0-9_$]{1}),narrationSuffix:([a-zA-Z0-9_$]{1})\\}\\)\\),${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.Mount,\\{when:(?:[a-zA-Z0-9_$]{1}),condition:!1\\},${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{title:(?:[a-zA-Z0-9_$]{1})\\("\\.useFlatWorldTitle"\\),description:(?:[a-zA-Z0-9_$]{1})\\("\\.useFlatWorldDescription"\\),value:(?:[a-zA-Z0-9_$]{1}),onChange:(?:[a-zA-Z0-9_$]{1}),disabled:(?:[a-zA-Z0-9_$]{1}),narrationSuffix:(?:[a-zA-Z0-9_$]{1}),offNarrationText:(?:[a-zA-Z0-9_$]{1}),onNarrationText:(?:[a-zA-Z0-9_$]{1}),onExpandNarrationHint:([a-zA-Z0-9_$]{1})\\},${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{title:([a-zA-Z0-9_$]{1})\\("\\.title"\\),customSelectionDescription:${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{preset:([a-zA-Z0-9_$]{1})\\}\\),options:([a-zA-Z0-9_$]{1}),value:([a-zA-Z0-9_$]{1}),onItemSelect:e=>([a-zA-Z0-9_$]{1})\\(([a-zA-Z0-9_$]{2})\\[e\\]\\),disabled:(?:[a-zA-Z0-9_$]{1}),wrapperRole:"neutral80",indented:!0,dropdownNarrationSuffix:([a-zA-Z0-9_$]{1})\\}\\)\\)\\)\\)`),
                 ...
                 */
                {
                    regex: new RegExp(`return ([a-zA-Z0-9_$]{1})\\.createElement\\(([a-zA-Z0-9_$]{2}|[a-zA-Z0-9_$]{1}\\.Unwrap),\\{data:([a-zA-Z0-9_$]{1})\\},\\(([a-zA-Z0-9_$]{1})=>([a-zA-Z0-9_$]{1})\\?([a-zA-Z0-9_$]{1})\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{onChange:([a-zA-Z0-9_$]{1}),value:([a-zA-Z0-9_$]{1}),title:([a-zA-Z0-9_$]{1})\\("\\.useFlatWorldTitle"\\),description:([a-zA-Z0-9_$]{1})\\("\\.useFlatWorldDescription"\\),disabled:([a-zA-Z0-9_$]{1}),offNarrationText:([a-zA-Z0-9_$]{1}),onNarrationText:([a-zA-Z0-9_$]{1}),narrationSuffix:([a-zA-Z0-9_$]{1})\\}\\):([a-zA-Z0-9_$]{1})\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{title:([a-zA-Z0-9_$]{1})\\("\\.useFlatWorldTitle"\\),description:([a-zA-Z0-9_$]{1})\\("\\.useFlatWorldDescription"\\),value:([a-zA-Z0-9_$]{1}),onChange:([a-zA-Z0-9_$]{1}),disabled:([a-zA-Z0-9_$]{1}),narrationSuffix:([a-zA-Z0-9_$]{1}),offNarrationText:([a-zA-Z0-9_$]{1}),onNarrationText:([a-zA-Z0-9_$]{1}),onExpandNarrationHint:([a-zA-Z0-9_$]{1})\\},([a-zA-Z0-9_$]{1})\\.createElement\\(([a-zA-Z0-9_$]{2},\\{(?:indentedPanel:!0,)?)title:([a-zA-Z0-9_$]{1})\\("\\.title"\\),customSelectionDescription:([a-zA-Z0-9_$]{1})\\.createElement\\(([a-zA-Z0-9_$]{2,3}),\\{selectedPreset:([a-zA-Z0-9_$]{1}),selectedPresetID:([a-zA-Z0-9_$]{1})\\}\\),options:([a-zA-Z0-9_$]{1}),value:([a-zA-Z0-9_$]{1}),onItemSelect:([a-zA-Z0-9_$]{1})=>([a-zA-Z0-9_$]{1})\\(([a-zA-Z0-9_$]{2,3})\\[([a-zA-Z0-9_$]{1})\\]\\),disabled:([a-zA-Z0-9_$]{1}),wrapperRole:"neutral80",((?:indented:!0,)?dropdownNarrationSuffix:[a-zA-Z0-9_$]{1})\\}\\)\\)\\)\\)\\}`),
        ...
        add8CrafterUtilitiesMainMenuButton: {
            ...
            0: [
                 ...
                 * - 1.21.90.20 preview (index-fe5c0.js)
                 */
                new RegExp(`${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(${extractedSymbolNames.facetHolder.replaceAll("$", "\\$")}\\.Mount,\\{when:([a-zA-Z0-9_$]{1})\\},${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.Fragment,null,(${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}.createElement\\([a-zA-Z0-9_$]{2},\\{open:[a-zA-Z0-9_$]{1},isOpen:[a-zA-Z0-9_$]{1}\\}\\),)?${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2})\\.Divider,null\\),${extractedSymbolNames.contextHolder.replaceAll("$", "\\$")}\\.createElement\\(([a-zA-Z0-9_$]{2}),\\{onClick:([eo]),screenAnalyticsId:([a-zA-Z0-9_$]{1})\\}\\)\\)\\)`),
            ],
        },
    };
        ...
        actions: [
            {
                id: "inject-facet-spy",
                context: "per_text_file",
                async action(currentFileContent, file) {
                    if (!/(?:index|gameplay|editor(?:-menu|-project)?)-[0-9a-f]{5,20}\.js$/.test(file.data?.filename))
                        return currentFileContent;
                    const origData = await file.getText();
                    ...
                    const isEditorMode = /editor-(?:menu|project)?[0-9a-f]{5,20}\.js$/.test(file.data?.filename);
                    /**
                     * The facet spy function that will be injected into the file.
                     */
                    function getFacetSpyFunction(preString) {
                        return `${preString}
            const facetList = [
                "core.animation",
                "core.customScaling",
        ...
        actions: [
            {
                ...
                action: async (currentFileContent, file) => {
                    ...
                    currentFileContent = currentFileContent.replace(/(?<=createElement\([a-zA-Z0-9_$]{3},\{[a-zA-Z0-9_$:,\s]*,showExportButton:)[a-zA-Z0-9_$]{1}(?=,|\})/g, "true");
                    return currentFileContent;
                },
            },
        ],
